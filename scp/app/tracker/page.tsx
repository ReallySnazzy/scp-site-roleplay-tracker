...
function TrackerContent() {
    ...
    const mode = searchParams.get('mode') as 'offline' | 'host' | 'client' | null
            ...
            peer.on('connection', (conn) => {
                ...
                conn.on('open', () => {
                    ...
                    if (logsRef.current.length > 0) {
                        conn.send({ type: 'SYNC_LOGS', logs: logsRef.current })
                    }
                })
...
export default function TrackerPage() {
    return (
        <Suspense fallback={<div className="min-h-screen bg-slate-950 flex items-center justify-center text-slate-500">Loading Tracker...</div>}>
            <TrackerContent />
        </Suspense>
    )
}
